...
// Float Event
// ============================================
// Broadcast event with a single float payload (speed change, fuel delta, etc.).
//
// USAGE: Create an asset (right-click → Create → Ludocore/Events/Float Event),
//        rename it semantically (SpeedBoostEvent.asset, FuelGainedEvent.asset),
//        and reference it from systems that raise or listen.
//
// NAMING: The class is generic; the ASSET NAME carries the meaning.
//         Never leave assets named "NewFloatEvent" — rename immediately.
//
...
namespace Ludocore
{
    [CreateAssetMenu(fileName = "NewFloatEvent", menuName = "Ludocore/Events/Float Event")]
    public class FloatEvent : ScriptableObject
    {
        ...
        /// <summary>Raised when Raise(float) is called. Subscribe in OnEnable, unsubscribe in OnDisable.</summary>
