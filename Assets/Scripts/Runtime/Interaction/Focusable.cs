...
namespace Ludocore
{
    ...
    public class Focusable : MonoBehaviour, IFocusable
    {
        ...
        [Tooltip("Optional broadcast event raised when this object becomes the player's focus. " +
                 "Any GameEventListener in the scene can react.")]
        [SerializeField] private GameEvent focusedGameEvent;

        [Tooltip("Optional broadcast event raised when this object stops being the player's focus.")]
        [SerializeField] private GameEvent unfocusedGameEvent;
        ...
        public void SetFocused(bool focused)
        {
            ...
            if (focused)
            {
                OnFocused?.Invoke();
                focusedEvent?.Invoke();
                if (focusedGameEvent) focusedGameEvent.Raise();
            }
            else
            {
                OnUnfocused?.Invoke();
                unfocusedEvent?.Invoke();
                if (unfocusedGameEvent) unfocusedGameEvent.Raise();
            }
        }
    }
}
